...
const MyCourses = ({ courses: coursesProp = [], user }) => {
  // Sử dụng courses từ props, không cần fetch lại
  const [courses] = useState(coursesProp);
  ...
  // Hàm fetch thumbnails - chỉ gọi khi cần
  ...
  const handleUnenroll = useCallback(async (courseId, e) => {
    ...
    try {
      ...
      // Reload page để cập nhật danh sách courses từ parent
  ...
  const isLoading = useMemo(() => {
    return !courses; // Chỉ loading nếu courses là undefined
  }, [courses]);
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
                ...
                <div className="relative h-40 bg-gradient-to-r from-blue-400 to-blue-600 overflow-hidden">
                  {thumbnailUrl ? (
                    <img 
                      ...
                      onLoad={() => {
                        // Đánh dấu thumbnail đã load thành công
        ...
        {/* Carousel Indicators - chỉ hiển thị nếu có nhiều hơn 1 course */}
        {courses.length > 1 && (
