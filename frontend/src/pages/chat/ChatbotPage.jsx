...
const ChatbotPage = () => { 
  ...
  const loadSessions = async () => { 
    try { 
      ...
      // Chỉ set currentSession nếu chưa có session nào được chọn
  ...
  const createNewSession = async () => { 
    ...
    // Tạo session mới ngay lập tức ở client side
    ...
    const newSession = {
      ...
      is_temp: true // Đánh dấu là session tạm thời
    };
    
    // Thêm session vào danh sách và set làm current
    setSessions(prev => [newSession, ...prev]);
    setCurrentSession(newSession);
    setMessages([]); // Xóa tin nhắn cũ

    try {
      // Gửi request tạo session thật lên server
      ...
      // Cập nhật session với ID thật từ server
      if (response.data.session_id) {
        setSessions(prev => prev.map(s => 
          s.id === tempSessionId 
            ? { ...s, id: response.data.session_id, is_temp: false }
            : s
        ));
        setCurrentSession(prev => prev.id === tempSessionId 
          ? { ...prev, id: response.data.session_id, is_temp: false }
          : prev
        );
      }
      
      // Reload sessions để đồng bộ
      await loadSessions();
    } catch (error) { 
      console.error('Failed to create session on server:', error); 
      // Nếu lỗi, vẫn giữ session tạm thời để người dùng có thể chat
      alert('Tạo session thành công, có thể có lỗi đồng bộ với server'); 
    } 
  }; 
  ...
  const sendMessage = async (e) => { 
    ...
    if (!currentSession) {
      ...
      // Đợi một chút để session được tạo
    ...
    try { 
      // Nếu là session tạm thời, tạo session thật trước
      let sessionIdToUse = currentSession.id;
      if (currentSession.is_temp) {
        ...
        if (response.data.session_id) {
          ...
          // Cập nhật session với ID thật
          setSessions(prev => prev.map(s => 
            s.id === currentSession.id 
              ? { ...s, id: sessionIdToUse, is_temp: false }
              : s
          ));
          setCurrentSession(prev => ({ ...prev, id: sessionIdToUse, is_temp: false }));
        }
      }
      ...
      setMessages(prev => prev.map(msg => 
        msg.id === aiMessageId 
          ? { ...msg, content: '❌ Lỗi khi gửi tin nhắn. Vui lòng thử lại.' } 
          : msg
      ));
    } finally { 
      setLoading(false); 
          ...
          {messages.length === 0 ? ( 
            <div className="h-full flex items-center justify-center text-gray-400"> 
              <div className="text-center"> 
                ...
                <p>{currentSession ? 'Bắt đầu một cuộc trò chuyện' : 'Chọn hoặc tạo một cuộc trò chuyện mới'}</p> 
              </div> 
            </div> 
          ) : ( 
